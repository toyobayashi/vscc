...
module.exports = async function copyTemplates(options) {
  ...
  if (root) {
    ...
    let pkgname = await (function () {
      return new Promise((resolve) => {
        rl.question(`Can not find package.json. Enter project name: (${defaultName})`, (answer) => {
          resolve(answer)
        })
      })
    })()
    ...
    const defaultPkgjson = {
      ...
      main: 'index.js',
      scripts: {
        'build:win': '.\\build.bat win32 static test',
        'build:unix': './build.sh test'
      },
      keywords: [],
    ...
    fs.writeFileSync(path.join(cwd, 'package.json'), JSON.stringify(defaultPkgjson, null, 2), 'utf8')
  }
  ...
  const gitignore = gitignorelist.join(require('os').EOL)
  const gitignorePath = path.join(cwd, '.gitignore')
  if (overwrite) {
    fs.writeFileSync(gitignorePath, gitignore, 'utf8')
  } else {
    if (!fs.existsSync(gitignorePath)) fs.writeFileSync(gitignorePath, gitignore, 'utf8')
  }
  ...
  if (process.platform === 'win32' && options && options.vscode) {
    await (async function findvs () {
      ...
      try {
        ...
        const clpath = path.join(vspath, 'VC/Tools/MSVC', ls[ls.length - 1], `bin/${process.arch === 'x64' ? 'Hostx64' : 'Hostx86'}/x86/cl.exe`)
